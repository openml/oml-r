.listOMLRunEvaluations = function(task.id = NULL, flow.id = NULL, run.id = NULL,
  ...
  # drop "all NA" columns
  na.ind = evals[, vlapply(.SD, function(x) !all(is.na(x)))]
  evals = evals[, na.ind, with = FALSE]
  ...
  setnames(evals, stri_replace_all_fixed(colnames(evals), "value_", ""))
  arr.ind = stri_detect_fixed(colnames(evals), "array_data_")
  arr.cols = colnames(evals)[arr.ind]
  setnames(evals, arr.cols, paste0(stri_replace_all_fixed(arr.cols, "array_data_", ""), "_array"))
  if (!show.array.measures) {
    evals[, which(arr.ind) := NULL]
  }
  ...
  setnames(evals, convertNamesOMLToR(names(evals)))

  # split flow.name into learner.name, version and flow.source
  if (extend.flow.name) {
    # extract flow.version
    flow.version = evals[, stri_match_last(flow.name, regex = "[[:digit:]]+\\.*[[:digit:]]*")]

    # extract flow.source
    src = c("weka", "mlr", "moa", "sklearn", "rm", "HubMiner", "classif", "regr", "surv", "openml")
    src = stri_paste("^", src)
    flow.source = evals[, stri_match_first(flow.name, regex = stri_paste(src, collapse = "|"))]
    ...
    learner.name = evals[, stri_replace_last(flow.name, replacement = "",
      regex = "\\([[:digit:]]+\\.*[[:digit:]]*\\)")]
    learner.name[!ind] = stri_replace_first(learner.name[!ind], replacement = "",
      regex = "^[[:alnum:]]+\\.*[.]")

    evals = setDT(append(evals, after = which(names(evals) == "flow.name"),
